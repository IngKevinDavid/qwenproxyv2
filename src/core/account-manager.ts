...
import { loadAccountsReal, updateAccountCooldown, invalidateAccountsCache as invalidateAccountsCacheSource } from './accounts.js'
...
export function getAccountsWithCooldownSync(): QwenAccount[] {
  const accounts = expandSingleAccountLanes(loadAccountsReal())
  const now = Date.now()
...
export function markAccountRateLimited(accountId: string, cooldownMs?: number, reason?: string): void {
  ...
  if (baseAccountId !== 'global') {
    try {
      ...
      console.error(`[AccountManager] Fallo al guardar el cooldown en la base de datos para la cuenta ${baseAccountId}:`, (err as Error).message)
    }
  }

  console.log(`[AccountManager] Cuenta ${accountId} marcada con límite de tasa activa. Cooldown hasta ${new Date(until).toISOString()}`)
}
...
export function clearAccountCooldown(accountId: string): void {
  ...
  if (baseAccountId !== 'global') {
    try {
      ...
      console.error(`[AccountManager] Fallo al limpiar el cooldown en la base de datos para la cuenta ${baseAccountId}:`, (err as Error).message)
    }
  }
}
