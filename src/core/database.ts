...
function runMigrations(db: Database.Database): void {
  ...
  try {
    db.exec(`ALTER TABLE accounts ADD COLUMN queue_position INTEGER DEFAULT 0;`)
  } catch { /* column may already exist */ }
  try {
    db.exec(`ALTER TABLE accounts ADD COLUMN used_tokens INTEGER DEFAULT 0;`)
  } catch { /* column may already exist */ }
  try {
    db.exec(`ALTER TABLE accounts ADD COLUMN used_tokens_plus INTEGER DEFAULT 0;`)
  } catch { /* column may already exist */ }
  try {
    db.exec(`ALTER TABLE accounts ADD COLUMN used_tokens_max INTEGER DEFAULT 0;`)
  } catch { /* column may already exist */ }
}
...
function encryptPlaintextPasswords(db: Database.Database): void {
  ...
  if (migrated > 0) {
    console.log(`[Database] Se encriptaron ${migrated} contraseñas en texto plano en la base de datos`)
  }
}
...
function migrateFromJson(db: Database.Database): void {
  ...
  try {
    ...
    fs.renameSync(jsonPath, jsonPath + '.bak')
    console.log(`[Database] Se migraron ${accounts.length} cuentas de accounts.json a SQLite`)
  } catch (err: any) {
    console.error('[Database] Falló la migración de accounts.json:', err.message)
  }
}
