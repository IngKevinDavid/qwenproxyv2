...
async function solveTmdChallengeIfPresent(page: Page, label: string): Promise<boolean> {
  ...
  if (!hasCaptcha) {
    console.warn(`[Qwen] Desafío TMD detectado para ${label}, pero no apareció ningún iframe de Baxia dentro de ${timeoutMs}ms.`);
    return false;
  }

  console.log(`[Qwen] Iframe de TMD Baxia detectado para ${label}; intentando resolverlo antes de reintentar...`);
...
export interface ClientConversationEntry {
  chatId: string;
  accountId: string;
  parentId: string | null;
  timestamp: number;
...
function cleanupStaleClientConversations() {
  const now = Date.now();
  for (const [key, entry] of clientConversationsMap.entries()) {
    if (now - entry.timestamp > SESSION_TTL_MS) {
      clientConversationsMap.delete(key);
    }
  }
}
...
export function getSessionParent(sessionId: string): string | null {
  return sessionStates.get(sessionId)?.parentId || null;
}
...
export async function disableNativeTools(accountId?: string): Promise<void> {
  ...
  try {
    ...
    console.log(`[Qwen] Desactivando herramientas nativas para ${cacheKey}...`);
    ...
    if (page && !page.isClosed() && page.url().includes('chat.qwen.ai')) {
      ...
      try {
        ...
        if (result.status && result.status < 400) {
          console.log(`[Qwen] Herramientas nativas desactivadas con éxito para ${cacheKey}.`);
          nativeToolsDisabled.add(cacheKey);
          return;
        }
        console.error(`[Qwen] Fallo al desactivar las herramientas nativas para ${cacheKey}: ${result.status} - ${result.body}`);
        return;
      } catch (err: any) {
    ...
    if (!response.ok) {
      ...
      console.error(`[Qwen] Fallo al desactivar las herramientas nativas para ${cacheKey}: ${response.status} - ${text}`);
    } else {
      console.log(`[Qwen] Herramientas nativas desactivadas con éxito para ${cacheKey}.`);
      nativeToolsDisabled.add(cacheKey);
    }
  } catch (err: any) {
    console.error(`[Qwen] Error al desactivar las herramientas nativas para ${cacheKey}: ${err.message}`);
  } finally {
...
export async function createQwenStream(
  ...
  const releaseStreamResources = () => {
    ...
    if (forcedChatId && accountId && accountId !== 'guest') {
  ...
  if (accountId === 'guest') {
    ...
    const { getBasicHeaders } = await import('./header-interceptor.js');
    ...
    chatHeaders = {
      ...
      'bx-ua': basicHeaders.bxUa || '',
      'bx-umidtoken': basicHeaders.bxUmidtoken || '',
    };
    assertAntiBotHeaders(chatHeaders, 'Forced stateful chat');
  } else {
    try {
  ...
  const actualParentId: string | null = forcedParentId !== undefined && forcedParentId !== null ? forcedParentId : (sessionStates.get(chatId)?.parentId || null);
