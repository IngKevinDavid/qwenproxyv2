...
async function refillPoolForAccount(accountId: string) {
  ...
  try {
    ...
    if (reused > 0) {
      console.log(`[WarmPool] Se reutilizaron ${reused} chats existentes no usados para la cuenta ${accountId}`);
    }
  } catch (err: any) {
    console.warn(`[WarmPool] Fallo al recuperar chats no usados para la cuenta ${accountId}:`, err.message);
  }
