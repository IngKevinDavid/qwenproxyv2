...
export async function getBasicHeaders(accountId?: string): Promise<{ cookie: string, userAgent: string, bxV: string, bxUa?: string, bxUmidtoken?: string }> {
  ...
  if (!bxUa || !bxUmidtoken) {
    console.log(`[Playwright] Falta bx-ua/bx-umidtoken para ${cacheKey}, activando interceptación de cabeceras...`);
    try {
  ...
  if (!guestPage) {
    ...
    try {
      ...
      if (keepSessionBtn) {
        ...
        console.log('[Playwright] Invitado: Se hizo clic en "Mantener sesión terminada"');
        await sleep(1000);
      }
    ...
    return await new Promise<Record<string, string>>((resolve, reject) => {
      const timeout = setTimeout(() => {
        resetBrowserProfile('guest', 'guest')
          .catch((err: any) => console.warn(`[Playwright] Fallo al restablecer el perfil de invitado tras timeout: ${err.message}`))
          .finally(() => reject(new Error('Timeout obteniendo las cabeceras de invitado')));
      }, config.timeouts.headers);

      const routeHandler = async (route: any, request: any) => {
        clearTimeout(timeout);
        const reqHeaders = request.headers();
        console.log('[Playwright] Invitado interceptó petición:', request.url());
        ...
        if (extractedHeaders['bx-ua']) {
          console.log('[Playwright] Invitado: bx-ua capturado con éxito');
          ...
          console.log('[Playwright] Invitado: Petición sin bx-ua, continuando ruta. Cabeceras:', Object.keys(reqHeaders));
...
async function _getQwenHeadersInternalOnce(forceNew = false, accountId?: string): Promise<{ headers: Record<string, string>, chatSessionId: string, parentMessageId: string | null }> {
  ...
  if (!isOnQwen) {
    console.log(`[Playwright] Navegando a la página estable de nuevo chat de Qwen para ${cacheKey}... (Actual: ${currentUrl})`);
  ...
  if (isLoginPage) {
    if (!accountId) {
      ...
      if (email && password) {
        console.log('[Playwright] Se detectó la página de inicio de sesión. Intentando inicio automático...');
        try {
          const { loginToQwen } = await import('./browser-manager.js');
          const loggedIn = await loginToQwen(email, password);
          if (!loggedIn) {
            throw new Error('loginToQwen devolvió false');
          }
          console.log('[Playwright] Inicio de sesión automático exitoso.');
        } catch (err: any) {
          console.error('[Playwright] Inicio de sesión automático falló:', err.message);
        }
      } else {
        console.warn('[Playwright] Se detectó la página de inicio de sesión pero QWEN_EMAIL/PASSWORD no están configurados en el .env');
      }
    } else {
      const { getAccountCredentials } = await import('../core/accounts.js');
      const creds = getAccountCredentials(getBaseAccountId(accountId));
      if (creds && creds.email && creds.password) {
        console.log(`[Playwright] Se detectó la página de inicio de sesión para la cuenta ${creds.email}. Intentando inicio de sesión...`);
  ...
  console.log(`[Playwright] Esperando entrada de chat para ${cacheKey}...`);
  const inputSelector = 'textarea:visible, [contenteditable="true"]:visible';
  await page.waitForSelector(inputSelector, { timeout: config.timeouts.page }).catch(() => {
    console.error(`[Playwright] Entrada de chat no encontrada para ${cacheKey}. URL actual:`, page.url());
    throw new Error(`Timeout esperando entrada de chat para ${cacheKey}. ¿Ha iniciado sesión?`);
  });
  ...
  try {
    return await new Promise<{ headers: Record<string, string>, chatSessionId: string, parentMessageId: string | null }>((resolve, reject) => {
      const timeout = setTimeout(async () => {
        console.error(`[Playwright] Timeout esperando cabeceras de Qwen para ${cacheKey}. URL actual:`, page.url());
        try {
          ...
          console.log(`[Playwright] Captura de pantalla de error guardada en ${screenshotPath}`);
        } catch (err: any) {
          console.error('[Playwright] Fallo al guardar captura de pantalla de error:', err.message);
        }
        reject(new Error(`Timeout esperando cabeceras de Qwen para ${cacheKey}`));
      }, config.timeouts.headers);

      console.log(`[Playwright] Configurando la interceptación de rutas para ${cacheKey}...`);
      const routeHandler = async (route: any, request: any) => {
        const reqHeaders = request.headers();
        let uiSessionId = '';
        ...
        if (!extractedHeaders.cookie || !extractedHeaders['bx-ua']) {
          console.log(`[Playwright] Petición interceptada sin cabeceras críticas para ${cacheKey}, omitiendo...`);
          await route.continue();
          return;
        }
        ...
        console.log(`[Playwright] Cabeceras interceptadas con éxito para ${cacheKey}.`);
        ...
        // Esperar activamente a que se procese y resuelva cualquier captcha de Baxia que salte por el envío simulado
        (async () => {
          try {
            // Tiempo de gracia para permitir que Qwen reaccione y renderice el captcha si aplica
            await sleep(1500);

            const { BAXIA_IFRAME_SELECTOR } = await import('./captcha-solver.js');
            let hasCaptcha = await page.locator(BAXIA_IFRAME_SELECTOR).first().isVisible().catch(() => false);
            let hasSlider = await page.locator('#nc_1_n1z, .btn_slide').first().isVisible().catch(() => false);
            let isPunishPage = page.url().includes('_____tmd_____/punish') || hasSlider;

            if (hasCaptcha || isPunishPage) {
              console.log(`[Playwright] Captcha detectado inmediatamente después de interceptar para ${cacheKey}. Esperando resolución completa...`);
              const startWait = Date.now();
              // Esperar hasta 25 segundos a que el watcher asíncrono resuelva el captcha y se limpie la pantalla
              while ((hasCaptcha || isPunishPage) && (Date.now() - startWait < 25000)) {
                await sleep(1000);
                if (page.isClosed()) break;
                hasCaptcha = await page.locator(BAXIA_IFRAME_SELECTOR).first().isVisible().catch(() => false);
                hasSlider = await page.locator('#nc_1_n1z, .btn_slide').first().isVisible().catch(() => false);
                isPunishPage = page.url().includes('_____tmd_____/punish') || hasSlider;
              }
              if (!hasCaptcha && !isPunishPage) {
                console.log(`[Playwright] Captcha resuelto y pantalla limpia con éxito en la interceptación para ${cacheKey}.`);
                // Tiempo de gracia adicional para consolidar el token en el backend de Qwen
                await sleep(1500);
              } else {
                console.warn(`[Playwright] Advertencia: El captcha para ${cacheKey} no se resolvió a tiempo durante el periodo de gracia.`);
              }
            }

            // Desactivar herramientas nativas una vez que el captcha se haya resuelto y la sesión esté limpia
            const m = await import('./qwen.js');
            await m.disableNativeTools(accountId).catch((err: any) => {
              console.error(`[Qwen] Error al desactivar herramientas nativas para ${cacheKey}:`, err.message);
            });

          } catch (err: any) {
            console.error(`[Playwright] Error durante la validación de captcha post-interceptación para ${cacheKey}:`, err.message);
          } finally {
      ...
      page.route('**/api/v2/chat/completions*', routeHandler).then(async () => {
        console.log(`[Playwright] Activando petición para ${cacheKey}...`);
        const inputSelector = 'textarea:visible, [contenteditable="true"]:visible';

        await humanType(page, inputSelector, 'Hello');
        console.log(`[Playwright] Se escribió texto simulando comportamiento humano para ${cacheKey}, esperando actualización de la interfaz...`);
        ...
        for (const selector of selectors) {
          try {
            ...
            if (btn && await btn.isVisible()) {
              console.log(`[Playwright] Intentando hacer clic en: ${selector}`);
            ...
            console.error(`[Playwright] Error al hacer clic en ${selector} para ${cacheKey}:`, e);
          }
        }

        if (!clicked) {
          console.log(`[Playwright] No se encontró o no se pudo hacer clic en el botón de enviar para ${cacheKey}, usando Enter como alternativa...`);
