...
test('session-parent-tracking: appends messages using response message_id as parent', async () => {
    ...
    const stream = new ReadableStream({
      start(c) {
        ...
        if (capturedPayloads.length === 1) {
          c.enqueue(new TextEncoder().encode(`data: {"choices": [{"delta": {"content": "Response 1", "phase": "answer"}}], "response_id": "${mockMessageId}"}\n\n`));
        }
        c.enqueue(new TextEncoder().encode('data: [DONE]\n\n'));
        c.close();
      }
  ...
  try {
    ...
    assert.strictEqual(capturedPayloads[1].messages[0].content, 'User: Turn 2\n\n', 'Should only send the last message in Stateful mode to optimize context');
  } finally {
    restore();
  }
});
